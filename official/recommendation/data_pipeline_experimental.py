...
class BaseDataConstructor(threading.Thread):
  ...
  def _construct_training_epoch(self):
    ...
    for (i, batch_fpath) in self._master_pool.imap_unordered(
        self._get_training_batch, map_args):
      tf.logging.info("In construct_training_epoch, batch_fpath: {}".format(batch_fpath))
  ...
  def _construct_eval_epoch(self):
    ...
    for (i, batch_fpath) in self._master_pool.imap_unordered(
      ...
      tf.logging.info("eval batch_fpath: {}".format(batch_fpath))
